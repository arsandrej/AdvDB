...
-- Change PKs to composite (required by TimescaleDB)
...
-- Outbound leg: count against source warehouse
SELECT
    pv.product_id,
    pv.brand_id,
    w_from.id                            AS warehouse_id,
...
FROM inventory_movements mv
    ...
    JOIN bins b_from            ON mv.from_bin_id = b_from.id
    JOIN locations l_from       ON b_from.location_id = l_from.id
    JOIN sections s_from        ON l_from.section_id = s_from.id
    JOIN warehouses w_from      ON s_from.warehouse_id = w_from.id
WHERE mv.from_bin_id IS NOT NULL
GROUP BY pv.product_id, pv.brand_id, w_from.id,
    time_bucket('1 day', mv.created_at)

UNION ALL

-- Inbound leg: count against destination warehouse
SELECT
    pv.product_id,
    pv.brand_id,
    w_to.id                              AS warehouse_id,
    time_bucket('1 day', mv.created_at)  AS day,
    SUM(mv.quantity)                     AS total_moved,
    COUNT(*)                             AS movement_count,
    COUNT(DISTINCT mv.inventory_transactions_id) AS transaction_count
FROM inventory_movements mv
    JOIN product_variants pv ON mv.product_variant_id = pv.id
    JOIN bins b_to            ON mv.to_bin_id = b_to.id
    JOIN locations l_to       ON b_to.location_id = l_to.id
    JOIN sections s_to        ON l_to.section_id = s_to.id
    JOIN warehouses w_to      ON s_to.warehouse_id = w_to.id
WHERE mv.to_bin_id IS NOT NULL
GROUP BY pv.product_id, pv.brand_id, w_to.id,
    time_bucket('1 day', mv.created_at)

WITH NO DATA;


-- Monthly cube (rolls up the daily cube)
...
GROUP BY product_id, brand_id, warehouse_id,
    time_bucket('1 month', day)
WITH NO DATA;


-- Plain materialized view for product attributes
...
-- FIX: start_offset increased from 3 months to 13 months
...
SELECT cron.schedule('refresh-attributes', '0 2 * * *',
                     'REFRESH MATERIALIZED VIEW cube_product_attributes');
...
WITH inventory_by_product AS (
    ...
    GROUP BY pv.product_id
     ...
     warehouse_turnover AS (
         SELECT
             w.name AS warehouse,
             SUM(cmm.total_moved) / NULLIF(SUM(ibp.total_on_hand), 0) AS turnover_ratio
         FROM cube_movements_monthly cmm
                  JOIN warehouses w           ON cmm.warehouse_id = w.id
                  LEFT JOIN inventory_by_product ibp ON ibp.product_id = cmm.product_id
...
-- 3. Top 25% red products by total moved quantity
WITH red_products AS (
    SELECT product_id
    FROM cube_product_attributes
    WHERE attribute_name = 'color'
      AND attribute_value = 'red'
),
     product_sales AS (
         SELECT
             cmm.product_id,
             SUM(total_moved) AS sales
         FROM cube_movements_monthly cmm
     ...
     ranked AS (
         SELECT
             ps.product_id,
             ps.sales,
             NTILE(4) OVER (ORDER BY ps.sales DESC) AS quartile
