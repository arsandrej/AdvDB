...
WITH role_levels (role_name, hierarchy_level) AS (VALUES ('Admin', 1),
                                                         ('Warehouse Manager', 1),
...
-- Each employee keeps only their most operational role
     employee_primary_role AS (SELECT DISTINCT ON (e.id) e.id AS employee_id,
                                                         rl.role_name,
                                                         rl.hierarchy_level
                               FROM EMPLOYEES e
...
-- Pool A: Warehouse Managers (for level-2 subordinates)
     pool_warehouse_mgr AS (SELECT employee_id,
...
-- ---------------------------------------------------------------------------
     ...
     subordinate_ranked AS (SELECT employee_id,
                                   role_name,
                                   hierarchy_level,
                                   ROW_NUMBER() OVER (
...
WITH primary_roles AS (SELECT DISTINCT ON (re.employees_id) re.employees_id AS employee_id,
                                                            r.name          AS role_name
                       FROM ROLES_EMPLOYEES re
                                ...
                                JOIN (VALUES ('Admin', 1),
                                             ...
                                             ('Receiving Clerk', 4),
                                             ('Shipping Coordinator', 4),
                                             ('Forklift Operator', 4)) AS rl(role_name, lvl) ON rl.role_name = r.name
                       ORDER BY re.employees_id, rl.lvl DESC)

SELECT pr_sub.role_name                AS subordinate_role,
