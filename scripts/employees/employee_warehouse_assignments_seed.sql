...
WITH ranked_employees AS (SELECT id,
                                 hired_at,
                                 terminated_at,
     ...
     warehouses_ranked AS (SELECT id,
                                  ROW_NUMBER() OVER (ORDER BY id) AS wrn
                           FROM WAREHOUSES),
     assigned AS (SELECT e.id                 AS employee_id,
...
warehouse_pairs AS (SELECT w1.id AS warehouse_id,
                           w2.id AS partner_id,
                           w1.city
                    FROM WAREHOUSES w1
                             JOIN WAREHOUSES w2
...
emp_primary AS (SELECT a.employee_id,
...
rotations AS (SELECT generate_series(1, 8) AS rot),
...
temp_raw AS (SELECT ep.employee_id,
...
SELECT employee_id,
       ...
       CURRENT_TIMESTAMP
FROM temp_raw
WHERE (term_date IS NULL OR start_date < term_date)
...
terminated AS (SELECT e.id                              AS employee_id,
...
historical_dated AS (SELECT employee_id,
...
WHERE h_end >= h_start -- sanity: end must not precede start
