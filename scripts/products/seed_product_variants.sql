...
JOIN  ATTRIBUTE_VALUES av1
ON av1.attribute_id = a1.id
...
WHERE av1.id IS NOT NULL
  AND (d.attr2_name IS NULL
   OR av2.id IS NOT NULL)
  AND (d.attr3_name IS NULL
   OR av3.id IS NOT NULL);
...
CREATE
TEMP TABLE product_base_price AS
...
WHERE CASE cat.name
          ...
          WHEN 'Desktop CPUs' THEN
              (dc.attr1_id IS NULL OR dc.attr1_id NOT IN (SELECT id
                                                          FROM ATTRIBUTES
                                                          WHERE name IN ('Screen Size', 'Panel Type', 'Switch Type',
                                                                         'Connectivity', 'Wattage', 'Kit Size',
                                                                         'RAM Capacity')))
                  AND (dc.attr2_id IS NULL OR dc.attr2_id NOT IN (SELECT id
                                                                  FROM ATTRIBUTES
                                                                  WHERE name IN
                                                                        ('Screen Size', 'Panel Type', 'Switch Type',
                                                                         'Connectivity', 'Wattage', 'Kit Size',
                                                                         'RAM Capacity')))
                  AND (dc.attr3_id IS NULL OR dc.attr3_id NOT IN (SELECT id
                                                                  FROM ATTRIBUTES
                                                                  WHERE name IN
                                                                        ('Screen Size', 'Panel Type', 'Switch Type',
                                                                         'Connectivity', 'Wattage', 'Kit Size',
          ...
          WHEN 'Server CPUs' THEN
              (dc.attr1_id IS NULL OR dc.attr1_id NOT IN (SELECT id
                                                          FROM ATTRIBUTES
                                                          WHERE name IN
                                                                ('Screen Size', 'Panel Type', 'Switch Type', 'Color',
                                                                 'Wattage', 'Kit Size', 'RAM Capacity')))
                  AND (dc.attr2_id IS NULL OR dc.attr2_id NOT IN (SELECT id
                                                                  FROM ATTRIBUTES
                                                                  WHERE name IN
                                                                        ('Screen Size', 'Panel Type', 'Switch Type',
                                                                         'Color', 'Wattage', 'Kit Size',
                                                                         'RAM Capacity')))
                  AND (dc.attr3_id IS NULL OR dc.attr3_id NOT IN (SELECT id
                                                                  FROM ATTRIBUTES
                                                                  WHERE name IN
          ...
          WHEN 'ATX Motherboards' THEN
              (dc.attr1_id IS NULL OR dc.attr1_id NOT IN (SELECT id
                                                          FROM ATTRIBUTES
                                                          WHERE name IN
                                                                ('Screen Size', 'Switch Type', 'Storage Capacity',
                                                                 'Kit Size', 'Wattage', 'RAM Capacity', 'Refresh Rate',
                                                                 'Resolution')))
                  AND (dc.attr2_id IS NULL OR dc.attr2_id NOT IN (SELECT id
                                                                  FROM ATTRIBUTES
                                                                  WHERE name IN ('Screen Size', 'Switch Type',
                                                                                 'Storage Capacity', 'Kit Size',
                                                                                 'Wattage', 'RAM Capacity',
          ...
          WHEN 'DDR5 RAM' THEN
              (dc.attr1_id IS NULL OR dc.attr1_id NOT IN (SELECT id
                                                          FROM ATTRIBUTES
                                                          WHERE name IN
                                                                ('Screen Size', 'Switch Type', 'Wattage', 'CPU Socket',
                                                                 'Storage Capacity', 'Refresh Rate', 'Resolution')))
                  AND (dc.attr2_id IS NULL OR dc.attr2_id NOT IN (SELECT id
                                                                  FROM ATTRIBUTES
                                                                  WHERE name IN
                                                                        ('Screen Size', 'Switch Type', 'Wattage',
                                                                         'CPU Socket', 'Storage Capacity',
                                                                         'Refresh Rate', 'Resolution')))
                  AND (dc.attr3_id IS NULL OR dc.attr3_id NOT IN (SELECT id
                                                                  FROM ATTRIBUTES
                                                                  WHERE name IN
          ...
          WHEN 'PCIe 4.0 NVMe SSDs' THEN
              (dc.attr1_id IS NULL OR dc.attr1_id NOT IN (SELECT id
                                                          FROM ATTRIBUTES
                                                          WHERE name IN ('Screen Size', 'Switch Type', 'CPU Socket',
                                                                         'Refresh Rate', 'Resolution', 'RAM Capacity',
                                                                         'Kit Size', 'Wattage')))
                  AND (dc.attr2_id IS NULL OR dc.attr2_id NOT IN (SELECT id
                                                                  FROM ATTRIBUTES
                                                                  WHERE name IN
                                                                        ('Screen Size', 'Switch Type', 'CPU Socket',
                                                                         'Refresh Rate', 'Resolution', 'RAM Capacity',
                                                                         'Kit Size', 'Wattage')))
                  AND (dc.attr3_id IS NULL OR dc.attr3_id NOT IN (SELECT id
                                                                  FROM ATTRIBUTES
          ...
          WHEN 'GeForce RTX 50 Series' THEN
              (dc.attr1_id IS NULL OR dc.attr1_id NOT IN (SELECT id
                                                          FROM ATTRIBUTES
                                                          WHERE name IN
                                                                ('Switch Type', 'Kit Size', 'CPU Socket', 'Speed',
                                                                 'Wattage', 'RAM Capacity', 'Storage Capacity')))
                  AND (dc.attr2_id IS NULL OR dc.attr2_id NOT IN (SELECT id
                                                                  FROM ATTRIBUTES
                                                                  WHERE name IN
                                                                        ('Switch Type', 'Kit Size', 'CPU Socket',
                                                                         'Speed', 'Wattage', 'RAM Capacity',
                                                                         'Storage Capacity')))
                  AND (dc.attr3_id IS NULL OR dc.attr3_id NOT IN (SELECT id
                                                                  FROM ATTRIBUTES
                                                                  WHERE name IN
          ...
          WHEN 'Gaming Monitors' THEN
              (dc.attr1_id IS NULL OR dc.attr1_id NOT IN (SELECT id
                                                          FROM ATTRIBUTES
                                                          WHERE name IN
                                                                ('Switch Type', 'CPU Socket', 'Kit Size', 'Wattage',
                                                                 'RAM Capacity', 'Storage Capacity', 'Speed')))
                  AND (dc.attr2_id IS NULL OR dc.attr2_id NOT IN (SELECT id
                                                                  FROM ATTRIBUTES
                                                                  WHERE name IN
                                                                        ('Switch Type', 'CPU Socket', 'Kit Size',
                                                                         'Wattage', 'RAM Capacity', 'Storage Capacity',
                                                                         'Speed')))
                  AND (dc.attr3_id IS NULL OR dc.attr3_id NOT IN (SELECT id
                                                                  FROM ATTRIBUTES
          ...
          WHEN 'Modular PSUs' THEN
              (dc.attr1_id IS NULL OR dc.attr1_id NOT IN (SELECT id
                                                          FROM ATTRIBUTES
                                                          WHERE name IN ('Screen Size', 'Switch Type', 'CPU Socket',
                                                                         'RAM Capacity', 'Storage Capacity',
                                                                         'Refresh Rate', 'Resolution', 'Kit Size',
                                                                         'Speed')))
                  AND (dc.attr2_id IS NULL OR dc.attr2_id NOT IN (SELECT id
                                                                  FROM ATTRIBUTES
                                                                  WHERE name IN
          ...
          WHEN 'TKL Keyboards' THEN
              (dc.attr1_id IS NULL OR dc.attr1_id NOT IN (SELECT id
                                                          FROM ATTRIBUTES
                                                          WHERE name IN ('Screen Size', 'Wattage', 'Storage Capacity',
                                                                         'RAM Capacity', 'CPU Socket', 'Refresh Rate',
                                                                         'Resolution', 'Kit Size', 'Speed')))
                  AND (dc.attr2_id IS NULL OR dc.attr2_id NOT IN (SELECT id
                                                                  FROM ATTRIBUTES
                                                                  WHERE name IN
                                                                        ('Screen Size', 'Wattage', 'Storage Capacity',
                                                                         'RAM Capacity', 'CPU Socket', 'Refresh Rate',
                                                                         'Resolution', 'Kit Size', 'Speed')))
                  AND (dc.attr3_id IS NULL OR dc.attr3_id NOT IN (SELECT id
                                                                  FROM ATTRIBUTES
          ...
          WHEN 'Gaming Mice' THEN
              (dc.attr1_id IS NULL OR dc.attr1_id NOT IN (SELECT id
                                                          FROM ATTRIBUTES
                                                          WHERE name IN
                                                                ('Screen Size', 'Wattage', 'CPU Socket', 'RAM Capacity',
                                                                 'Storage Capacity', 'Switch Type', 'Refresh Rate',
                                                                 'Resolution', 'Kit Size', 'Speed')))
                  AND (dc.attr2_id IS NULL OR dc.attr2_id NOT IN (SELECT id
                                                                  FROM ATTRIBUTES
                                                                  WHERE name IN ('Screen Size', 'Wattage', 'CPU Socket',
                                                                                 'RAM Capacity', 'Storage Capacity',
                                                                                 'Switch Type', 'Refresh Rate',
                                                                                 'Resolution', 'Kit Size', 'Speed')))
                  AND (dc.attr3_id IS NULL OR dc.attr3_id NOT IN (SELECT id
                                                                  FROM ATTRIBUTES
                                                                  WHERE name IN ('Screen Size', 'Wattage', 'CPU Socket',
          ...
          WHEN 'Gaming Headsets' THEN
              (dc.attr1_id IS NULL OR dc.attr1_id NOT IN (SELECT id
                                                          FROM ATTRIBUTES
                                                          WHERE name IN ('Screen Size', 'CPU Socket', 'RAM Capacity',
                                                                         'Storage Capacity', 'Switch Type', 'Wattage',
                                                                         'Refresh Rate', 'Resolution', 'Kit Size',
                                                                         'Speed')))
                  AND (dc.attr2_id IS NULL OR dc.attr2_id NOT IN (SELECT id
                                                                  FROM ATTRIBUTES
                                                                  WHERE name IN
                                                                        ('Screen Size', 'CPU Socket', 'RAM Capacity',
                                                                         'Storage Capacity', 'Switch Type', 'Wattage',
                                                                         'Refresh Rate', 'Resolution', 'Kit Size',
                                                                         'Speed')))
                  AND (dc.attr3_id IS NULL OR dc.attr3_id NOT IN (SELECT id
...
SELECT 'PRODUCT_VARIANTS' AS tbl, COUNT(*) AS rows
