...
-- ============================================================================
-- ============================================================================
WITH sample_names (base_name, description) AS (VALUES ('Floral Print Maxi Dress', 'Elegant floral print maxi dress'),
...
VALUES ('Size', 'string', NULL, true),
       ('Color', 'string', NULL, true),
       ('Material', 'string', NULL, false),
       ('Sleeve Length', 'string', NULL, false),
       ('Neckline', 'string', NULL, false),
       ('Fit', 'string', NULL, false),
       ('Pattern', 'string', NULL, false),
       ('Occasion', 'string', NULL, false),
       ('Season', 'string', NULL, false),
       ('Gender', 'string', NULL, false),
       ('Metal Type', 'string', NULL, false),
       ('Gemstone', 'string', NULL, false);
...
SELECT p.id,
       COALESCE(
               CASE
                   WHEN p.id % 30 BETWEEN 0 AND 5 THEN (SELECT id FROM CATEGORIES WHERE parent_id = 6 ORDER BY random()
               LIMIT 1)
...
FROM PRODUCTS p ON CONFLICT (product_id, category_id) DO NOTHING;

-- ============================================================================

-- ============================================================================
WITH color_subset AS (SELECT value   AS color,
                             CASE
                                 WHEN value = 'Black' THEN 'BLK'
                                 WHEN value = 'White' THEN 'WHT'
                                 WHEN value = 'Red' THEN 'RED'
                                 WHEN value = 'Blue' THEN 'BLU'
                                 WHEN value = 'Green' THEN 'GRN'
                                 WHEN value = 'Yellow' THEN 'YLW'
                                 WHEN value = 'Pink' THEN 'PNK'
                                 WHEN value = 'Purple' THEN 'PUR'
                                 WHEN value = 'Orange' THEN 'ORG'
                                 WHEN value = 'Brown' THEN 'BRN'
                                 WHEN value = 'Grey' THEN 'GRY'
                                 WHEN value = 'Navy' THEN 'NVY'
                                 WHEN value = 'Beige' THEN 'BGE'
                                 WHEN value = 'Burgundy' THEN 'BUR'
                                 WHEN value = 'Olive' THEN 'OLV'
                                 WHEN value = 'Teal' THEN 'TEA'
                                 WHEN value = 'Coral' THEN 'CRL'
                                 WHEN value = 'Mint' THEN 'MNT'
                                 WHEN value = 'Lavender' THEN 'LAV'
                                 WHEN value = 'Peach' THEN 'PCH'
                                 WHEN value = 'Khaki' THEN 'KHK'
                                 WHEN value = 'Denim Blue' THEN 'DNM'
                                 WHEN value = 'Charcoal' THEN 'CHR'
                                 WHEN value = 'Ivory' THEN 'IVR'
                                 WHEN value = 'Maroon' THEN 'MRN'
                                 ELSE UPPER(SUBSTRING(value, 1, 3))
                                 END AS color_code
...
SELECT
    p.id AS product_id, cs.color_code, ss.size_code, (p.id % 35) + 1 AS brand_id, LPAD((FLOOR(RANDOM() * 1000000000000))::TEXT, 12, '0') AS barcode, ROUND((RANDOM() * 150 + 15):: NUMERIC, 2) AS price, CASE WHEN p.id % 10 = 0 THEN ROUND((RANDOM() * 2 + 0.1):: NUMERIC, 2) ELSE NULL END AS weight, CASE WHEN RANDOM() < 0.9 THEN 'ACTIVE' ELSE 'INACTIVE' END AS status, ROW_NUMBER() OVER (ORDER BY p.id, cs.color, ss.size) AS global_seq
FROM PRODUCTS p
...
INSERT
INTO PRODUCT_VARIANTS (product_id, sku, brand_id, barcode, price, weight, status)
SELECT product_id,
       UPPER(
...
SET status = CASE WHEN RANDOM() < 0.05 THEN 'OUT_OF_STOCK' WHEN RANDOM() < 0.02 THEN 'DISCONTINUED' ELSE status END;
...
FROM PRODUCT_VARIANTS pv
         CROSS JOIN generate_series(1, 3) pos(seq)
WHERE pv.id % 3 = 0
...
INSERT
INTO VARIANT_ATTRIBUTES (product_variant_id, attribute_value_id, attribute_id)
