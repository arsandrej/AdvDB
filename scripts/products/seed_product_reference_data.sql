...
CREATE
TEMP TABLE stg_brands (
    name VARCHAR(255)
);

CREATE
TEMP TABLE stg_categories (
    name        VARCHAR(255),
    parent_name VARCHAR(255)
);

CREATE
TEMP TABLE stg_attributes (
    name                 VARCHAR(255),
...
CREATE
TEMP TABLE stg_attribute_values (
...
DELETE
FROM stg_attribute_values
WHERE ctid NOT IN (SELECT MIN(ctid) FROM stg_attribute_values GROUP BY attribute_name, value);
...
SELECT TRIM(name)
FROM stg_brands
WHERE TRIM(name) <> '' ON CONFLICT (name) DO NOTHING;
...
FROM stg_categories
WHERE TRIM(COALESCE(parent_name, '')) = '' ON CONFLICT (parent_id, name) DO NOTHING;
...
WHERE TRIM(s.value) <> '' ON CONFLICT (attribute_id, value) DO NOTHING;
