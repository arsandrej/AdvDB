...
RETURNS VOID AS $$
    UPDATE LOCATIONS
    SET row_number = p_row_number, column_number = p_column_number,
        level_number = p_level_number, location_code = p_location_code
    WHERE id = p_id;
$$ LANGUAGE sql;

-- Bins
CREATE OR REPLACE FUNCTION create_bin(
...
CREATE OR REPLACE FUNCTION update_bin(
    p_id       BIGINT,
    p_bin_code TEXT,
    p_capacity INT
)
RETURNS VOID AS $$
    UPDATE BINS
    SET bin_code = p_bin_code, capacity = p_capacity
    WHERE id = p_id;
$$ LANGUAGE sql;
...
RETURNS VOID AS $$
    ...
    SET first_name = p_first_name, last_name = p_last_name, email = p_email,
        ...
        employment_status = p_employment_status, manager_id = p_manager_id
...
RETURNS VOID AS $$
    DELETE FROM ROLES_EMPLOYEES
    WHERE roles_id = p_role_id AND employees_id = p_employee_id;
$$ LANGUAGE sql;
